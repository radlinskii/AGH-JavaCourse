...
public class CSVReader {
    ...
    LocalTime getTime(int columnIndex, String format){
        if (isMissing(columnIndex))
            return LocalTime.parse("00:00", DateTimeFormatter.ofPattern("HH:mm"));;
        return LocalTime.parse(current[columnIndex],DateTimeFormatter.ofPattern(format));
    }

    LocalTime getTime(String element, String format){
        if (isMissing(element))
            return LocalTime.parse("00:00:00", DateTimeFormatter.ofPattern(format));;
        return LocalTime.parse(current[columnLabelsToInt.get(element)],DateTimeFormatter.ofPattern(format));
    }
